...
func server(args ...string) {
	...
	stunservers := set.String("stun", "stun:relay.webwormhole.com", "list of STUN server addresses to tell clients to use")
	...
	handler := func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Host == "webwormhole.io" {
			// Hard code the redirect in here for the transition. We can move
			http.Redirect(w, r, fmt.Sprintf("https://webwormhole.com%v", r.URL.Path), 307)
			return
		}
